...
const ENDPOINT_URL_v2 = "https://www.grubhub.com/search"

//params
//?orderMethod=delivery&locationMode=DELIVERY&facetSet=umamiV2&pageSize=20&hideHateos=true&searchMetrics=true
//&latitude=37.99491882&longitude=-78.48793030&preciseLocation=true&facet=open_now%3Atrue&sortSetId=umamiV2&sponsoredSize=3&countOmittingTimes=true
...
Search.prototype.url = function(opts) {
  ...
  let resultsV2 = ENDPOINT_URL_v2;

  var options = ["orderMethod=delivery&locationMode=DELIVERY&facetSet=umamiV2", 
                "hideHateos=true&searchMetrics=true",
                "preciseLocation=true&sortSetId=umamiV2&sponsoredSize=3&countOmittingTimes=true"];
  if(opts.onlyOpen) options.push('facet=open_now%3Atrue');
  // if(opts.sort) options.push('sort=' + opts.sort);
  if (this._where) {
    options.push("latitude=" + this._where.latitude);
    options.push("longitude=" + this._where.longitude);
  }

  if(options.length);
    resultsV2 += '?' + options.join('&');
